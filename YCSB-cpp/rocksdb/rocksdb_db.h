...
public:
  ...
  void GetOrPrintDBStatus(std::map<std::string, std::string> *status_map,
                          bool should_print, bool reset_stats) {
    std::map<std::string, std::string> cache_status;
    assert(db_ != nullptr);
    db_->GetMapProperty(rocksdb::DB::Properties::kBlockCacheEntryStats,
                        &cache_status);

    std::shared_ptr<rocksdb::Statistics> statistics =
        db_->GetOptions().statistics;
    uint64_t block_cache_hit, block_cache_hit_lm, block_cache_hit_rm,
        block_cache_miss;
    uint64_t block_cache_data_hit, block_cache_data_miss;
    uint64_t block_cache_compression_dict_hit,
        block_cache_compression_dict_miss;
    uint64_t memtable_hit, memtable_miss;
    double block_cache_hit_ratio, block_cache_hit_lm_ratio,
        block_cache_hit_rm_ratio, block_cache_data_hit_ratio,
        block_cache_compression_dict_hit_ratio, memtable_hit_ratio;
    if (statistics != nullptr) {
      block_cache_hit = statistics->getTickerCount(rocksdb::BLOCK_CACHE_HIT);
      block_cache_hit_lm =
          statistics->getTickerCount(rocksdb::BLOCK_CACHE_HIT_LM);
      block_cache_hit_rm =
          statistics->getTickerCount(rocksdb::BLOCK_CACHE_HIT_RM);
      block_cache_miss = statistics->getTickerCount(rocksdb::BLOCK_CACHE_MISS);
      block_cache_hit_ratio =
          (double)block_cache_hit / (block_cache_hit + block_cache_miss) * 100;
      block_cache_hit_lm_ratio = (double)block_cache_hit_lm /
                                 (block_cache_hit + block_cache_miss) * 100;
      block_cache_hit_rm_ratio = (double)block_cache_hit_rm /
                                 (block_cache_hit + block_cache_miss) * 100;
      block_cache_data_hit =
          statistics->getTickerCount(rocksdb::BLOCK_CACHE_DATA_HIT);
      block_cache_data_miss =
          statistics->getTickerCount(rocksdb::BLOCK_CACHE_DATA_MISS);
      block_cache_data_hit_ratio =
          (double)block_cache_data_hit /
          (block_cache_data_hit + block_cache_data_miss) * 100;
      block_cache_compression_dict_hit =
          statistics->getTickerCount(rocksdb::BLOCK_CACHE_COMPRESSION_DICT_HIT);
      block_cache_compression_dict_miss = statistics->getTickerCount(
          rocksdb::BLOCK_CACHE_COMPRESSION_DICT_MISS);
      block_cache_compression_dict_hit_ratio =
          (double)block_cache_compression_dict_hit /
          (block_cache_compression_dict_hit +
           block_cache_compression_dict_miss) *
          100;
      ...
      memtable_hit_ratio =
          (double)memtable_hit / (memtable_hit + memtable_miss) * 100;
    }

    // try to print stats
    if (should_print) {
      printf("Block cache entry stats(count,bytes,percent): "
             "DataBlock(%s, %s, %s), LMDataBlock(_, %s, %s), RMDataBlock(_, "
             "%s, %s), IndexBlock(%s, %s, %s), FilterBlock(%s, %s, %s), "
             "Misc(%s, %s, %s)\n",
             cache_status["count.data-block"].c_str(),
             cache_status["bytes.data-block"].c_str(),
             cache_status["percent.data-block"].c_str(),
             cache_status["bytes.lm-data-block"].c_str(),
             cache_status["percent.lm-data-block"].c_str(),
             cache_status["bytes.rm-data-block"].c_str(),
             cache_status["percent.rm-data-block"].c_str(),
             cache_status["count.index-block"].c_str(),
             cache_status["bytes.index-block"].c_str(),
             cache_status["percent.index-block"].c_str(),
             cache_status["count.filter-block"].c_str(),
             cache_status["bytes.filter-block"].c_str(),
             cache_status["percent.filter-block"].c_str(),
             cache_status["count.misc"].c_str(),
             cache_status["bytes.misc"].c_str(),
             cache_status["percent.misc"].c_str());
      printf("Block cache %s capacity: %s\n", cache_status["id"].c_str(),
             cache_status["capacity"].c_str());
      ...
      if (statistics != nullptr) {
        printf("block cache hit ratio: %lu/%lu = %.2lf%%\n", block_cache_hit,
               block_cache_hit + block_cache_miss, block_cache_hit_ratio);
        printf("block cache hit lm ratio: %lu/%lu = %.2lf%%\n",
               block_cache_hit_lm, block_cache_hit + block_cache_miss,
               block_cache_hit_lm_ratio);
        printf("block cache hit rm ratio: %lu/%lu = %.2lf%%\n",
               block_cache_hit_rm, block_cache_hit + block_cache_miss,
               block_cache_hit_rm_ratio);
        ...
        printf("memtable hit ratio: %lu/%lu = %.2lf%%\n", memtable_hit,
               memtable_hit + memtable_miss, memtable_hit_ratio);
    ...
    if (status_map != nullptr) {
      // block cache
      (*status_map)["block_cache_data_percent"] =
          cache_status["percent.data-block"];
      if (statistics != nullptr) {
        (*status_map)["block_cache_hit"] = std::to_string(block_cache_hit);
        (*status_map)["block_cache_hit_lm"] =
            std::to_string(block_cache_hit_lm);
        (*status_map)["block_cache_hit_rm"] =
            std::to_string(block_cache_hit_rm);
        (*status_map)["block_cache_miss"] = std::to_string(block_cache_miss);
        (*status_map)["block_cache_hit_ratio"] =
            std::to_string(block_cache_hit_ratio);
        (*status_map)["block_cache_hit_lm_ratio"] =
            std::to_string(block_cache_hit_lm_ratio);
        (*status_map)["block_cache_hit_rm_ratio"] =
            std::to_string(block_cache_hit_rm_ratio);
        (*status_map)["block_cache_data_hit"] =
            std::to_string(block_cache_data_hit);
        (*status_map)["block_cache_data_miss"] =
            std::to_string(block_cache_data_miss);
        (*status_map)["block_cache_data_hit_ratio"] =
            std::to_string(block_cache_data_hit_ratio);
      }
      // memtable
      if (statistics != nullptr) {
        (*status_map)["memtable_hit"] = std::to_string(memtable_hit);
        (*status_map)["memtable_miss"] = std::to_string(memtable_miss);
        (*status_map)["memtable_hit_ratio"] =
            std::to_string(memtable_hit_ratio);
      }
    }
    // try to reset stats
