...
namespace {
  ...
  const std::string PROP_MIN_WRITE_BUFFER_NUMBER_TO_MERGE = "rocksdb.min_write_buffer_number_to_merge";
...
void RocksdbDB::GetOptions(const utils::Properties &props, rocksdb::Options *opt,
  ...
  if (options_file != "") {
    ...
    if (cache_size > 0) {
      block_cache = rocksdb::NewRMLRUCache(cache_size, table_cache_numshardbits, false);
    ...
    int min_write_buffer_number_to_merge = std::stoi(props.GetProperty(PROP_MIN_WRITE_BUFFER_NUMBER_TO_MERGE,
