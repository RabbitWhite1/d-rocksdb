...
const std::string PROP_MIN_WRITE_BUFFER_NUMBER_TO_MERGE =
    "rocksdb.min_write_buffer_number_to_merge";
const std::string PROP_MIN_WRITE_BUFFER_NUMBER_TO_MERGE_DEFAULT = "2";
...
const std::string PROP_TABLE_CACHE_NUMSHARDBITS =
...
void RocksdbDB::GetOptions(
  ...
  if (options_file != "") {
    rocksdb::Status s =
        rocksdb::LoadOptionsFromFile(options_file, env, opt, cf_descs);
    if (!s.ok()) {
    ...
    size_t write_buffer_size = std::stoul(props.GetProperty(
    ...
    int table_cache_numshardbits = std::stoi(props.GetProperty(
        PROP_TABLE_CACHE_NUMSHARDBITS, PROP_TABLE_CACHE_NUMSHARDBITS_DEFAULT));
    double rm_ratio =
        std::stod(props.GetProperty(PROP_RM_RATIO, PROP_RM_RATIO_DEFAULT));
    ...
    if (cache_size > 0) {
      ...
      // table_cache_numshardbits, false, 0.5);
    ...
    if (compressed_cache_size > 0) {
      block_cache =
          rocksdb::NewLRUCache(cache_size, table_cache_numshardbits, false);
    ...
    int min_write_buffer_number_to_merge = std::stoi(
        props.GetProperty(PROP_MIN_WRITE_BUFFER_NUMBER_TO_MERGE,
                          PROP_MIN_WRITE_BUFFER_NUMBER_TO_MERGE_DEFAULT));

    // statistics
    std::shared_ptr<rocksdb::Statistics> statistics =
        rocksdb::CreateDBStatistics();
    statistics->set_stats_level(rocksdb::StatsLevel::kExceptTimeForMutex);
