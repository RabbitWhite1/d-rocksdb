...
struct RMAsyncDirectRequest {
  ...
  int wait() {
    int ret;
    // if (!is_read) {
    //   auto lm_block = (Block *)lm_value;
    //   printf(
    //       "wait done(is_read=%d), lm_region: [%p, %p), rm_region: "
    //       "[0x%lx,0x%lx)\n",
    //       is_read, lm_block->data(), lm_block->data() + lm_block->size(),
    //       rm_region->addr, rm_region->addr + rm_region->size);
    // } else {
    //   printf("wait done(is_read=%d)\n", is_read);
    // }
    ret = rdma_async_request->wait();
