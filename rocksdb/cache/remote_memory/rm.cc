...
RemoteMemory::RemoteMemory(size_t size)
    : rm_size_(size) {
  rm_addr_ = 0x1000;  // TODO: allocate remotely and send back the addr.
...
struct RMRegion *RemoteMemory::split_and_use_region(RMRegion *region, size_t first_size) {
    // TODO: move this method outside to RemoteMemory (which is more convenient to manager the linked list)
    // return the possible new free region, which is also a possible new head
...
uint64_t RemoteMemory::rmalloc(size_t size) {
  ...
  {
    RMRegion *free_region = free_head_;
    while (free_region != nullptr) {
      printf("try to allocate 0x%lx from region(0x%lx, 0x%lx)\n", size,
            free_region->addr, free_region->size);
...
RMRegion *RemoteMemory::extract_from_free_list(RMRegion *region) {
  if (region->prev_free) {
...
RMRegion *RemoteMemory::prepend_free(RMRegion *region) {
...
void RemoteMemory::print() {
  ...
  while (region != nullptr) {
    if (region->is_free) {
      printf("-->{[0x%lx, 0x%lx), 0x%lx, free}", region->addr, region->addr + region->size, region->size);
    } else {
      printf("-->{[0x%lx, 0x%lx), 0x%lx, used}", region->addr, region->addr + region->size, region->size);
    }
    region = region->next;
  }
  putchar('\n');
